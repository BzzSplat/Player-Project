...
public class PhysicsGunInteractionBehavior : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject grabEffect;
    private GameObject activeGrabEffect;
    [SerializeField]
    float grabStrength = 0.06f, maxGrabWeight = 1000f;
    ...
    private void Start()
    {
        _camera = GetComponent<Weapon>().camera;
        PlayerTransform = GetComponent<Weapon>().Holder.transform;
	...
	private void Update ()
    {
        ...
        if (_grabbedRigidbody == null && !_justReleased)
        {
            ...
            if (Physics.Raycast(ray, out hit, _maxGrabDistance, _grabLayer))
            {
                ...
                if (hit.rigidbody != null && hit.rigidbody.mass <= maxGrabWeight)
                {
                    ...
                    _wasRotateFroze                     = _grabbedRigidbody.freezeRotation;
    ...
    private void FixedUpdate()
    {
        if (_grabbedRigidbody)
        {
            ...
            // Calculate force
            var force = toDestination / Time.fixedDeltaTime * (_grabbedRigidbody.mass * grabStrength) / _grabbedRigidbody.mass; //0.3f, 500mass = 30f
    ...
    private void ReleaseObject()
    {
        ...
        Destroy(activeGrabEffect);
        activeGrabEffect = null;
